...
def favorite_activities():
    ...
    activities = []
    for day in days:
        question = input(f'What is your favorite thing to do on {day}? ')
...
def temp_by_day():
    ...
    for day in days:
        temp = int(input(f'What is the temperature outside {day}? '))
        if temp < 50:
...
def temp_by_day_continuous():
    ...
    # Spring has sprung!
    temp = int((input(f'What is the temperature outside? ')))
    while temp < 65:
    ...
    # hit control + C to stop the program. No harm done to your computer :slightly_smiling_face: